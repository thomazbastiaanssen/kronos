...
```{r showpairwisemodel, echo=FALSE}
...
```{r multi-plots, echo=FALSE}

plot_list<- list() #make an empty list

for(q in 1:length(out_list)){
  
  #extract plotting info within the loop
  out = out_list[[q]]
  
  #save plot into relevant position in list
  plot_list[[q]] <- out %>% {
    
    #your ggplot script here  
    
    gg_kronos_sinusoid(out)
   }
  #end of ggplot script
}

#to plot & save the feature graphs to a pdf:
for (r in 1:length(out_list)) {
  ggsave(filename=paste0("Figure_",(names(out_list[r])),"plot.pdf"), plot=plot_list[[r]], device="pdf")
}
